...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // We use useMemo to create it only once, effectively treating it as a singleton.
    const dataProvider = React.useMemo(() => new CompositeDataProvider(), []);

    // Token Refresh Logic
    useEffect(() => {
        const checkAuth = async () => {
            if (isInitialized && state.currentUser && state.currentUser.tokens) {
                // Check if we need to refresh (simple check: if we have a refresh token)
                // A more robust check would look at expiry_date, but let's try to refresh on load to be safe
                // or just trust the current token until it fails.
                // Better strategy for this use case: proactively refresh on app launch if possible.

                if (state.currentUser.tokens.refresh_token && window.electronAPI) {
                    try {
                        console.log("Attempting to refresh Google Tokens...");
                        const refreshResult = await window.electronAPI.invoke('auth:refresh-tokens', state.currentUser.tokens.refresh_token);
                        if (refreshResult && refreshResult.user) {
                            console.log("Tokens refreshed successfully.");
                            dispatch({ type: 'SET_USER', payload: refreshResult.user });
                        }
                    } catch (e) {
                        console.error("Failed to refresh token on startup:", e);
    ...
    useEffect(() => {
        const loadState = async () => {
            try {
                const savedState = await dataProvider.load();
