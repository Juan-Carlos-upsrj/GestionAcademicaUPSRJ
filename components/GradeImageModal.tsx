...
const GradeImageModal: React.FC<GradeImageModalProps> = ({
    ...
    const [viewMode, setViewMode] = useState<'p1' | 'p2' | 'final' | 'complete'>('p1');
            ...
            <div className="flex flex-col gap-4">
                <div className="flex justify-between items-center">
                    <div className="flex gap-2">
                        <button onClick={() => setViewMode('p1')} className={`px-3 py-1 rounded-md text-sm font-semibold ${viewMode === 'p1' ? 'bg-primary text-white' : 'bg-surface-secondary text-text-secondary'}`}>Parcial 1</button>
                        <button onClick={() => setViewMode('p2')} className={`px-3 py-1 rounded-md text-sm font-semibold ${viewMode === 'p2' ? 'bg-primary text-white' : 'bg-surface-secondary text-text-secondary'}`}>Parcial 2</button>
                        <button onClick={() => setViewMode('final')} className={`px-3 py-1 rounded-md text-sm font-semibold ${viewMode === 'final' ? 'bg-primary text-white' : 'bg-surface-secondary text-text-secondary'}`}>Final</button>
                        <button onClick={() => setViewMode('complete')} className={`px-3 py-1 rounded-md text-sm font-semibold ${viewMode === 'complete' ? 'bg-indigo-600 text-white shadow-md ring-1 ring-indigo-500' : 'bg-surface-secondary text-text-secondary'}`}>Completo</button>
                    </div>
                    <div className="flex gap-2">
                        <Button size="sm" variant="secondary" onClick={handleCopy}><Icon name="copy" className="w-4 h-4" /> Copiar Imagen</Button>
                        <Button size="sm" variant="secondary" onClick={handleDownload}><Icon name="download-cloud" className="w-4 h-4" /> Guardar/Compartir PNG</Button>
                        <Button size="sm" onClick={handleDownloadPDF}><Icon name="download-cloud" className="w-4 h-4" /> Guardar/Compartir PDF</Button>
                    </div>
                </div>
                <div className="overflow-auto bg-slate-100 p-4 rounded-lg border border-border-color flex justify-center">
                    <div ref={tableRef} className="bg-white p-6 rounded-none shadow-none min-w-[800px] text-slate-800">
                        <div className="mb-4 border-b-2 border-indigo-500 pb-2">
                            ...
                            <p className="text-xs text-slate-400 mt-1 uppercase tracking-wider">{viewMode === 'final' ? 'Promedios Finales' : viewMode === 'p1' ? 'Reporte Primer Parcial' : 'Reporte Segundo Parcial'}</p>
                        </div>
                        <table className="w-full text-sm border-collapse">
                            <thead>
                                <tr className="border-b border-slate-300">
                                    <th className="p-2 text-left font-bold text-slate-600">Alumno</th>
                                    {viewMode !== 'final' && viewMode !== 'complete' && partialEvaluations.map(ev => <th key={ev.id} className="p-2 text-center font-semibold text-slate-600 text-xs">{ev.name} <br /><span className="font-normal opacity-70">({ev.maxScore})</span></th>)}
                                    ...
                                    {viewMode === 'complete' ? (
                                        <>
                                            ...
                                            {evaluations.filter(e => e.partial === 1).map(ev => <th key={`head-p1-${ev.id}`} className="p-1 text-center text-[10px] text-slate-500 font-normal rotate-0 max-w-[60px] truncate">{ev.name}</th>)}
                                            <th className="p-2 text-center font-bold text-indigo-700 bg-indigo-50 border-r border-slate-300">P1</th>
                                            {evaluations.filter(e => e.partial === 2).map(ev => <th key={`head-p2-${ev.id}`} className="p-1 text-center text-[10px] text-slate-500 font-normal rotate-0 max-w-[60px] truncate">{ev.name}</th>)}
                                            <th className="p-2 text-center font-bold text-indigo-700 bg-indigo-50 border-r border-slate-300">P2</th>
                                            <th className="p-2 text-center font-bold text-indigo-900 bg-indigo-100">Final</th>
                                        </>
                                    ) : viewMode === 'final' ? (
                                        <>
                                            <th className="p-2 text-center font-bold text-slate-600">Asist %</th>
                            ...
                            <tbody>
                                {group.students.map((student, idx) => {
                                    ...
                                    return (
                                        <tr key={student.id} className={`border-b border-slate-100 ${idx % 2 === 0 ? 'bg-slate-50/50' : ''} ${showFailAlert ? 'bg-rose-50' : showInfoAlert ? 'bg-amber-50' : ''}`}>
                                            <td className="p-2 font-medium text-slate-700">
                                                <div className="flex items-center gap-1">
                                                    <span className={showFailAlert ? 'text-rose-700 font-bold' : showInfoAlert ? 'text-amber-700 font-bold' : ''}>{student.name}</span>
                                                    {isCriticalFail && settings.failByAttendance && <span className="text-[7px] font-black bg-rose-600 text-white px-1 rounded-sm uppercase tracking-tighter shrink-0">Baja Asist.</span>}
                                                    {isCriticalFail && !settings.failByAttendance && <span className="text-[7px] font-black bg-blue-500 text-white px-1 rounded-sm uppercase tracking-tighter shrink-0">Asist. Baja</span>}
                                            ...
                                            {viewMode !== 'final' && partialEvaluations.map(ev => <td key={ev.id} className="p-2 text-center text-slate-600">{grades[student.id]?.[ev.id] ?? '-'}</td>)}
                                            ...
                                            {viewMode === 'complete' ? (
                                                <>
                                                    <td className={`p-2 text-center font-black ${showFailAlert ? 'text-rose-600' : showInfoAlert ? 'text-amber-600' : 'text-slate-400'}`}>{globalAtt.toFixed(0)}%</td>

                                                    {/* P1 Data */}
                                                    {evaluations.filter(e => e.partial === 1).map(ev => (
                                                        <td key={`p1-${ev.id}`} className="p-1 text-center text-xs text-slate-500 border-l border-slate-100">{grades[student.id]?.[ev.id] ?? '-'}</td>
                                                    ))}
                                                    <td className={`p-2 text-center font-bold border-r border-slate-200 ${getGradeColor(p1Avg)}`}>{p1Avg?.toFixed(1) || '-'}</td>

                                                    {/* P2 Data */}
                                                    {evaluations.filter(e => e.partial === 2).map(ev => (
                                                        <td key={`p2-${ev.id}`} className="p-1 text-center text-xs text-slate-500 border-l border-slate-100">{grades[student.id]?.[ev.id] ?? '-'}</td>
                                                    ))}
                                                    <td className={`p-2 text-center font-bold border-r border-slate-200 ${getGradeColor(p2Avg)}`}>{p2Avg?.toFixed(1) || '-'}</td>

                                                    <td className={`p-2 text-center font-black text-lg ${showFailAlert ? 'text-rose-600' : getGradeColor(finalAvg)}`}>{finalAvg?.toFixed(1) || '-'}</td>
                                                </>
                                            ) : viewMode === 'final' ? (
                                                <>
                                                    <td className={`p-2 text-center font-black ${showFailAlert ? 'text-rose-600' : showInfoAlert ? 'text-amber-600' : 'text-slate-400'}`}>{globalAtt.toFixed(0)}%</td>
                                            ...
                                            ) : <td className={`p-2 text-center font-bold bg-indigo-50 ${getGradeColor(viewMode === 'p1' ? p1Avg : p2Avg)}`}>{(viewMode === 'p1' ? p1Avg : p2Avg)?.toFixed(1) || '-'}</td>}
                                        </tr>
                                    );
                                })}
                            </tbody>
                        </table>
                        <div className="mt-4 pt-2 border-t border-slate-200 flex justify-between items-center"><span className="text-xs text-slate-400">Generado el {new Date().toLocaleDateString()}</span><span className="text-xs font-bold text-indigo-700">Gestión IAEV</span></div>
                    </div>
                </div>
            </div>
        </Modal>
